...
int Input::Algorithm_A() {
	...
	if (m >= n) {
	    vector< vector< int > > LPT_schedule;
		int LPT_makespan_1 = LPT(array, LPT_schedule, 0);

		vector< vector< int > > johnson_schedule;
		int johnsons_makespan = johnsons(array, johnson_schedule, LPT_makespan_1);
	...
	// split job schedule; one set has m jobs, the other has n-m jobs
...
int Input::Algorithm_B1() {
	...
	return max(T_2 + A - J_l[1], T_3 + J_l[3]);
}
...
int Input::Algorithm_B2_3() {
	...
	if (B3_make == 0)
		B3_make = max( max(A_time, B_time), C_time);
    ...
    if (B2_make == -1) {
    	...
    	if (B3_make == -1)
		    ...
		    return min(B2_make, B3_make);
    }
}

float Input::run() {
	int LB = lower_bound();
	int A_make = Algorithm_A();
	int B1_make = Algorithm_B1();
	int B2_3_make = Algorithm_B2_3();

	if (A_make == -1) {
		if (B1_make == -1)
			return B2_3_make/(float)LB;
		else
			return min(B1_make, B2_3_make)/(float)LB;
	}
	else {
		if (B1_make == -1)
			return min(A_make, B2_3_make)/(float)LB;
		else
			return min(min(A_make, B1_make), B2_3_make)/(float)LB;
	}
}